...
EditKeyAction() {
    ...
    Run("`"D:\Programs Files\Visual Studio Code.lnk`" `"" LogFile "`"")
    return
}
...
LogKey_Mouse(key) {
    ...
    Log((MouseMode != "relative" ? ";" : "") "MouseClick(`"" k "`", " (tempRelativeX - RelativeX) ", " (tempRelativeY -
        RelativeY) ",,, `"D`", `"R`") `;relative")
    ...
    if (InStr(r, ",,, `"D`")") && Abs(X2 - X1) + Abs(Y2 - Y1) < 5)
        ...
        Log((MouseMode == "window" || MouseMode == "relative" ? ";" : "") "MouseClick(`"" k "`", " (X + X2 - X1) ", " (
            Y + Y2 - Y1) ",,, `"U`") `;screen")
